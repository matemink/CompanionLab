...
std::pair<std::string, bool> normalized_name(
...
std::optional<ParsedLine> parse_line(
    std::string line,
...
std::optional<presentation::BoardTypeMatch>
BoardTypeCatalog::resolve(const std::uint16_t board_type) const {
    const auto found = entries_.find(board_type);
    if (found == entries_.end()) {
        return std::nullopt;
    }
    return presentation::BoardTypeMatch{
...
std::size_t BoardTypeCatalog::alias_count() const {
    return alias_count_;
}

void BoardTypeCatalog::add(
    const std::uint16_t board_type,
    std::string name,
