...
domain::CameraExtrinsics parse_document(const Json& document) {
    if (document.at("schema_version").get<std::uint32_t>() != 1U ||
        document.at("frame_from").get<std::string>() !=
            "camera_optical" ||
    ...
    const auto& translation =
        document.at("camera_origin_in_body_m");
    domain::CameraExtrinsics extrinsics{
    ...
    domain::validate_camera_extrinsics(extrinsics);
    return extrinsics;
}

}  // namespace

domain::CameraExtrinsics CameraExtrinsicsLoader::from_file(
...
domain::CameraExtrinsics CameraExtrinsicsLoader::from_stream(
    std::istream& input
) {
    try {
        ...
        throw std::runtime_error(
            "invalid camera extrinsics JSON: " +
