...
domain::ImagePoint undistort_image_point(
    ...
    if (!std::isfinite(distorted.x_px) ||
        !std::isfinite(distorted.y_px)) {
        throw std::invalid_argument(
            "distorted image point must be finite"
        );
    }

    const double distorted_x =
        (distorted.x_px - calibration.cx_px) /
        calibration.fx_px;
    const double distorted_y =
        (distorted.y_px - calibration.cy_px) /
        calibration.fy_px;
    ...
    for (int iteration = 0; iteration < iteration_count;
        ...
        const double radius_fourth =
            radius_squared * radius_squared;
        const double radius_sixth =
            radius_fourth * radius_squared;
        const double radial_scale =
            1.0 + k1 * radius_squared +
            k2 * radius_fourth + k3 * radius_sixth;
        if (!std::isfinite(radial_scale) ||
            std::abs(radial_scale) < minimum_radial_scale) {
            throw std::runtime_error(
                "camera distortion cannot be inverted"
            );
        }
