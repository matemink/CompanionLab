...
bool is_fresh(
    const std::optional<TimePoint>& observed_at,
    TimePoint now,
...
std::string VehicleSnapshot::to_json() const {
    ...
    write_optional_number(
        output,
        autopilot_metadata
            ? std::optional{autopilot_metadata->firmware_major}
            : std::nullopt
    );
    ...
    write_optional_number(
        output,
        autopilot_metadata
            ? std::optional{autopilot_metadata->firmware_minor}
            : std::nullopt
    );
    ...
    write_optional_number(
        output,
        autopilot_metadata
            ? std::optional{autopilot_metadata->firmware_patch}
            : std::nullopt
    );
    ...
    write_optional_number(
        output,
        autopilot_metadata
            ? std::optional{autopilot_metadata->capabilities}
            : std::nullopt
    );
    ...
    write_optional_number(
        output,
        autopilot_metadata
            ? std::optional{autopilot_metadata->board_version}
            : std::nullopt
    );
    ...
    write_optional_number(
        output,
        autopilot_metadata
            ? std::optional{autopilot_metadata->vendor_id}
            : std::nullopt
    );
    ...
    write_optional_number(
        output,
        autopilot_metadata
            ? std::optional{autopilot_metadata->product_id}
            : std::nullopt
    );
    output << ",\"warnings\":[";
...
void VehicleState::on_heartbeat(
...
void VehicleState::on_global_position(
    const std::int32_t relative_altitude_mm,
    const TimePoint now
) {
    std::scoped_lock lock(mutex_);
    last_global_position_ = now;
    relative_altitude_m_ =
        static_cast<double>(relative_altitude_mm) / 1000.0;
}

void VehicleState::on_local_position(
    const float north_m,
    const float east_m,
    const float down_m,
    const TimePoint now
...
void VehicleState::update_battery_locked(
...
void VehicleState::on_battery_arming_voltage(
...
void VehicleState::on_autopilot_metadata(
    const AutopilotMetadata metadata
) {
    std::scoped_lock lock(mutex_);
    autopilot_metadata_ = metadata;
}

void VehicleState::on_system_status(
    const std::uint32_t sensors_enabled,
...
void VehicleState::on_status_text(
    ...
    const auto existing = std::find_if(
        warnings_.begin(),
        warnings_.end(),
        [&text](const WarningEntry& warning) {
...
VehicleSnapshot VehicleState::snapshot(const TimePoint now) {
    ...
    if (gps_fresh) {
        ...
        result.gps_ready =
            gps_fix_type_.has_value() && *gps_fix_type_ >= 3;
    }
    ...
    const bool battery_sensor_healthy =
        system_status_fresh &&
        (sensors_enabled_ & kBatterySensorFlag) != 0 &&
        (sensors_healthy_ & kBatterySensorFlag) != 0;
    const bool has_battery_warning = std::any_of(
        warnings_.begin(),
        warnings_.end(),
        [](const WarningEntry& warning) {
    ...
    const bool battery_fresh =
        is_fresh(last_battery_, now, kBatteryTimeout);
    if (battery_fresh) {
        ...
        result.battery_arming_voltage_v =
            battery_arming_voltage_v_;
        ...
        result.battery_ready =
            voltage_meets_arming_threshold &&
            battery_sensor_healthy &&
            !has_battery_warning &&
    ...
    result.system_health_ok =
        system_status_fresh &&
        (sensors_enabled_ & ~sensors_healthy_) == 0;

    const bool has_recent_warning = std::any_of(
        warnings_.begin(),
        warnings_.end(),
        [now](const WarningEntry& warning) {
