...
double row_dot(
    ...
    for (std::size_t column = 0; column < 3U; ++column) {
        result += rotation[first * 3U + column] *
            rotation[second * 3U + column];
    }
    return result;
}

double determinant(const std::array<double, 9>& rotation) {
    return
        rotation[0] *
            (rotation[4] * rotation[8] -
             rotation[5] * rotation[7]) -
        rotation[1] *
            (rotation[3] * rotation[8] -
             rotation[5] * rotation[6]) +
        rotation[2] *
            (rotation[3] * rotation[7] -
             rotation[4] * rotation[6]);
}

}  // namespace

void validate_camera_extrinsics(
    ...
    if (std::abs(
            determinant(extrinsics.rotation_camera_to_body) - 1.0
        ) > kRotationTolerance) {
        throw std::invalid_argument(
...
BodyFramePosition camera_to_body_frd(
    ...
    if (!std::isfinite(camera_position.right_m) ||
        ...
        throw std::invalid_argument(
            "camera target position must be finite"
        );
    }
    ...
    for (std::size_t row = 0; row < 3U; ++row) {
        for (std::size_t column = 0; column < 3U; ++column) {
            body[row] +=
                extrinsics.rotation_camera_to_body[row * 3U + column] *
                camera[column];
        }
    }
    ...
    return {
        .forward_m = body[0] +
            extrinsics.camera_origin_in_body.forward_m,
        .right_m = body[1] +
            extrinsics.camera_origin_in_body.right_m,
        .down_m = body[2] +
            extrinsics.camera_origin_in_body.down_m,
    };
}
