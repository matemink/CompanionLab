...
constexpr std::uint32_t kGcsFailsafeContinuationMask =
    (1U << 1U) | (1U << 4U);
constexpr double kMinimumTimeoutS = 2.0;
constexpr double kMaximumTimeoutS = 10.0;

template <typename T>
std::optional<T> exact_integer(
...
void CompanionLinkFailsafe::observe_vehicle(
...
void CompanionLinkFailsafe::on_parameter(
...
void CompanionLinkFailsafe::reset(
    const std::optional<std::uint8_t> system_id
) {
    snapshot_ = {};
    ...
    if (system_id.has_value()) {
        snapshot_.phase =
            CompanionLinkFailsafePhase::reading_parameters;
        snapshot_.detail =
...
void CompanionLinkFailsafe::validate() {
    ...
    if (snapshot_.parameters_received <
        snapshot_.parameters_required) {
        snapshot_.phase =
            CompanionLinkFailsafePhase::reading_parameters;
        snapshot_.detail =
            "Reading ArduPilot failsafe parameters (" +
            std::to_string(snapshot_.parameters_received) + "/" +
            std::to_string(snapshot_.parameters_required) + ")";
        return;
    }

    const auto action_value = exact_integer<std::uint8_t>(
        raw_action_,
        static_cast<double>(
            std::numeric_limits<std::uint8_t>::max()
        )
    );
    const auto options_value = exact_integer<std::uint32_t>(
        raw_options_,
        static_cast<double>(
            std::numeric_limits<std::uint32_t>::max()
        )
    );
    const auto gcs_system_id = exact_integer<std::uint8_t>(
        raw_gcs_system_id_,
        static_cast<double>(
            std::numeric_limits<std::uint8_t>::max()
        )
    ...
    snapshot_.action = action_value.has_value() &&
                               *action_value <= 7U
        ...
        : std::nullopt;
    snapshot_.timeout_s =
        raw_timeout_s_.has_value() &&
                std::isfinite(*raw_timeout_s_)
            ? raw_timeout_s_
            : std::nullopt;
    ...
    if (*snapshot_.action != ArduPilotGcsFailsafeAction::land) {
        snapshot_.detail =
            "FS_GCS_ENABLE must be 5 (Always LAND)";
        return;
    }
    if (!raw_timeout_s_.has_value() ||
        !std::isfinite(*raw_timeout_s_) ||
        *raw_timeout_s_ < kMinimumTimeoutS ||
        *raw_timeout_s_ > kMaximumTimeoutS) {
        snapshot_.detail =
            "FS_GCS_TIMEOUT must be between 2 and 10 seconds";
        return;
    }
    ...
    if ((*options_value & kGcsFailsafeContinuationMask) != 0U) {
        snapshot_.detail =
            "FS_OPTIONS must not bypass GCS failsafe in Auto or "
            "pilot modes";
        return;
    }
    if (!gcs_system_id.has_value() ||
        *gcs_system_id == 0U) {
