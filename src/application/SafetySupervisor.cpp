...
bool valid_target(const domain::BodyFramePosition& target) {
    return std::isfinite(target.forward_m) &&
           std::isfinite(target.right_m) &&
           std::isfinite(target.down_m) &&
           target.down_m > 0.0;
}

}  // namespace

SupervisedMotion SafetySupervisor::supervise(
