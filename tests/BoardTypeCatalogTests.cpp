...
void parser_preserves_aliases_and_prefers_reserved_names() {
    ...
    require(
        unquoted.has_value() &&
            unquoted->preferred_name ==
                "Unquoted Friendly Board",
        "unquoted multi-word reserved names must be parsed"
    );
    require(
        !catalog.resolve(65000).has_value(),
...
void pinned_ardupilot_table_is_complete() {
    ...
    require(
        catalog.alias_count() == 369,
        "the pinned ArduPilot table must keep all 369 rows"
    );
    require(
        catalog.board_type_count() == 358,
...
void ambiguous_board_is_honest_in_the_console() {
    ...
    require(
        output.find(
            "TARGET_HW_PX4_FMU_V2 / ID 9 / SILICON 0 / 3 ALIASES"
        ) != std::string::npos,
