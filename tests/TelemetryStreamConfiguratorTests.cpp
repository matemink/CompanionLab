...
mavlink_command_long_t decode_command(
    ...
    for (const auto byte : bytes) {
        ...
        if (framing == MAVLINK_FRAMING_OK) {
            require(
                parsed_message.msgid == MAVLINK_MSG_ID_COMMAND_LONG,
...
void requests_each_required_stream_sequentially() {
    ...
    require(
        decode_command(*system_health).param1 ==
    ...
    require(
        decode_command(*position).param1 ==
    ...
    require(
        decode_command(*local_position).param1 ==
    ...
    require(
        decode_command(*attitude).param1 ==
    ...
    require(
        snapshot.completed_requests == snapshot.total_requests,
...
void retries_then_fails_when_ack_is_missing() {
    ...
    require(
        !configurator.update(true, 1, after_retries).has_value(),
...
void rejected_command_fails_immediately() {
    ...
    require(
        snapshot.failure_result == MAV_RESULT_UNSUPPORTED,
