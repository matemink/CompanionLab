...
void partial_heartbeat_is_reassembled() {
    ...
    decoder.ingest(
        std::span<const std::uint8_t>{bytes.data(), split},
        now
    );
    require(
        !state.snapshot(now).connected,
...
void minimum_message_set_updates_vehicle_state() {
    ...
    require(
        snapshot.battery_remaining_pct == 88,
        "SYS_STATUS must update battery state"
    );
    require(
        snapshot.battery_arming_voltage_v == 10.5,
        "PARAM_VALUE must update BATT_ARM_VOLT"
    );
    require(
        snapshot.relative_altitude_m == 2.5,
        "GLOBAL_POSITION_INT must update relative altitude"
    );
    require(
        snapshot.local_north_m == 12.5 &&
            snapshot.local_east_m == -3.0 &&
            snapshot.local_down_m == -2.5,
        "LOCAL_POSITION_NED must update local route state"
    );
    require(
        snapshot.roll_rad.has_value() &&
            snapshot.pitch_rad.has_value() &&
...
void autopilot_version_is_unpacked_into_domain_metadata() {
    ...
    require(
        metadata.board_version == version.board_version &&
...
void companion_heartbeat_does_not_replace_autopilot() {
    ...
    const auto snapshot = state.snapshot(now);
    require(
        snapshot.component_id == MAV_COMP_ID_AUTOPILOT1,
        "companion heartbeat must not replace autopilot identity"
    );
    require(
        snapshot.vehicle_type == MAV_TYPE_QUADROTOR,
...
void command_ack_is_forwarded_to_its_handler() {
    ...
    require(
        observed->command == MAV_CMD_SET_MESSAGE_INTERVAL &&
