...
void physical_calibration_loads_exact_values() {
    ...
    const auto calibration =
        onboard_autonomy::adapters::vision::
            CameraCalibrationLoader::from_file(path);

    require(
        calibration.camera_model == "imx708_wide" &&
            calibration.image_width == 640U &&
            calibration.image_height == 480U &&
            std::abs(calibration.fx_px - 543.625989446945) <
                1.0e-9 &&
            std::abs(calibration.distortion[4] -
                     0.4749880305495665) < 1.0e-12,
...
void failed_quality_gate_is_rejected() {
    ...
    try {
        static_cast<void>(
            onboard_autonomy::adapters::vision::
                CameraCalibrationLoader::from_stream(input)
        );
...
void inconsistent_camera_matrix_is_rejected() {
    ...
    require(
        rejected,
        "redundant matrix and scalar intrinsics must agree"
    );
}
