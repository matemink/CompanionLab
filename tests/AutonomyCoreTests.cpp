...
using onboard_autonomy::application::DecisionEngine;
using onboard_autonomy::application::MotionSafetyStatus;
using onboard_autonomy::application::SafetySupervisor;
using onboard_autonomy::application::WorldState;
...
WorldState tracking_world(const TimePoint now) {
    return {
        ...
        .vehicle_armed = true,
        .vehicle_system_id = 1,
        .landing_target = BodyFramePosition{
            .forward_m = 0.4,
            .right_m = -0.2,
            .down_m = 8.0,
        },
...
void decision_engine_requires_a_current_target() {
    ...
    require(
        desired.has_value() &&
            desired->landing_target.forward_m == 0.4 &&
    ...
    require(
        desired->valid_until - desired->created_at ==
...
void safety_supervisor_rejects_degraded_world_state() {
    ...
    require(
        supervisor.supervise(world, desired, start).status ==
...
void safety_supervisor_rejects_non_finite_target() {
    ...
    const TimePoint start{};
    auto world = tracking_world(start);
    world.landing_target->forward_m =
