...
mavlink_message_t decode_message(
    const std::vector<std::uint8_t>& bytes
) {
    mavlink_message_t receive_buffer{};
    mavlink_status_t receive_status{};
...
void message_interval_request_uses_command_long() {
    ...
    require(
        command.param2 == static_cast<float>(interval_microseconds),
...
void movement_and_precision_messages_use_documented_frames() {
    ...
    require(
        move.coordinate_frame == MAV_FRAME_LOCAL_OFFSET_NED &&
            move.type_mask == 3576 &&
            move.x == 10.0F &&
            move.y == -4.0F &&
            move.z == 0.0F,
    ...
    require(
        target_message.msgid == MAVLINK_MSG_ID_LANDING_TARGET,
