...
onboard_autonomy::domain::CameraExtrinsics downward_camera() {
    return {
...
void downward_camera_maps_optical_axes_to_body_frd() {
    const auto body =
        onboard_autonomy::domain::camera_to_body_frd(
            {
                .right_m = 0.20,
                .down_m = -0.40,
                .forward_m = 1.50,
    ...
    require(
        std::abs(body.forward_m - 0.40) < 1.0e-12 &&
...
void invalid_rotation_is_rejected() {
    ...
    bool rejected = false;
    try {
        onboard_autonomy::domain::validate_camera_extrinsics(
            extrinsics
        );
    ...
    require(
        rejected,
        "non-rigid camera rotations must be rejected"
    );
}
