...
onboard_autonomy::domain::TargetObservation observation(
...
onboard_autonomy::application::TargetTracker tracker() {
    using namespace std::chrono_literals;
    return onboard_autonomy::application::TargetTracker{
        {
...
void tracker_requires_confirmation_and_smooths_position() {
    ...
    target_tracker.update(
        std::vector{observation(0, 0.0, 0.0, 1.0)},
        start
    );
    ...
    target_tracker.update(
        std::vector{observation(0, 1.0, -0.2, 1.2)},
        start + 20ms
    );
    target_tracker.update(
        std::vector{observation(0, 1.0, -0.2, 0.8)},
        start + 40ms
    );
...
void tracker_does_not_jump_between_tags() {
    ...
    target_tracker.update(
        std::vector{observation(1, 0.4, 0.0, 1.0, 90.0)},
        start + 100ms
    );
...
void tracker_rejects_unsafe_pose_observations() {
    ...
    require(
        target_tracker.snapshot(now).phase ==
            onboard_autonomy::application::
                TargetTrackPhase::searching,
