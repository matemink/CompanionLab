...
using onboard_autonomy::application::ArduPilotGcsFailsafeAction;
using onboard_autonomy::application::CompanionLinkFailsafe;
...
CompanionLinkFailsafe configured_policy(
...
void exact_land_policy_is_accepted() {
    ...
    require(
        snapshot.phase == CompanionLinkFailsafePhase::accepted,
        "Always LAND policy must be accepted"
    );
    require(
        snapshot.action == ArduPilotGcsFailsafeAction::land &&
            snapshot.timeout_s == 3.0 &&
            snapshot.options == 0U &&
...
void unsafe_parameter_combinations_are_rejected() {
    require(
        configured_policy(0.0).snapshot().phase ==
            CompanionLinkFailsafePhase::rejected,
        "disabled GCS failsafe must be rejected"
    );
    require(
        configured_policy(5.0, 120.0).snapshot().phase ==
            CompanionLinkFailsafePhase::rejected,
        "excessive companion-loss timeout must be rejected"
    );
    const auto nonfinite = configured_policy(
        5.0,
        std::numeric_limits<double>::quiet_NaN()
    ).snapshot();
    require(
        nonfinite.phase == CompanionLinkFailsafePhase::rejected &&
            !nonfinite.timeout_s.has_value(),
        "non-finite timeout must be rejected without invalid JSON data"
    );
    require(
        configured_policy(5.0, 3.0, 2.0).snapshot().phase ==
            CompanionLinkFailsafePhase::rejected,
        "Auto continuation bit must be rejected"
    );
    require(
        configured_policy(5.0, 3.0, 16.0).snapshot().phase ==
            CompanionLinkFailsafePhase::rejected,
        "pilot-mode continuation bit must be rejected"
    );
    require(
        configured_policy(5.0, 3.0, 0.0, 255.0)
                .snapshot()
                .phase == CompanionLinkFailsafePhase::rejected,
